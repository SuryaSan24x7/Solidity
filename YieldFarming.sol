...
contract YieldFarming is ERC20 {
    struct Pool {
        ...
        uint yield_percent;
        uint pool_balance;
        // address => [deposit value, last reward time]
    ...
    Pool[] public pools;
    // total deposit - to check for whales
    ...
    function withdrawWei(uint poolId, uint amount) public {
        unchecked {
            ...
            // reset get reward time if we withdraw everything
            if (pools[poolId].deposits[msg.sender][0] == 0) { pools[poolId].deposits[msg.sender][1] = block.timestamp;}
